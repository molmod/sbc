...
def train(
    ...
    while epoch < max_num_epochs:
        ...
        if epoch % eval_interval == 0:
            ...
            elif log_errors == 'cross_entropy':
                s = "Epoch {:4}: loss={:9.4f}  ".format(epoch, valid_loss)
                for phase in model.phases:
                    s += 'acc({}): {:4.3f}'.format(phase, eval_metrics['acc({})'.format(phase)])
                    s += ' | '
                s += 'CE: {:5.3f}'.format(eval_metrics['CE'])
...
def evaluate(
    ...
    accuracies = {p: None for p in model.phases}
    cv_forces_list = []
    logits_list = []
    phase_list = []
    ...
    for batch in data_loader:
        ...
        if output.get('logits', None) is not None:
            logits_computed = True
            logits_list.append(output['logits'])
            cv_forces_list.append(output['CV_forces'])
    ...
    if logits_computed:
        cv_forces = torch.cat(cv_forces_list, dim=0)
        logits = torch.cat(logits_list, dim=0)
        phases = torch.cat(phase_list, dim=0)
        forces = torch.cat(fs_list, dim=0)
        prediction = torch.argmax(logits, dim=1)
        for i, phase in enumerate(model.phases):
            mask = torch.eq(phases, torch.tensor([i]))
            total = torch.sum(mask)
            if total > 0:
                true_p = torch.sum(torch.eq(prediction[mask], torch.tensor([i])))
                accuracies[phase] = true_p / total
            aux['acc({})'.format(phase)] = accuracies[phase]

        # compute loss contributions
        mask = torch.logical_not(torch.isnan(phases))
        aux['CE'] = loss_fn.ce_loss(logits[mask], phases[mask].to(torch.long))

        mask = torch.logical_not(torch.sum(torch.isnan(forces), dim=1))
        if torch.any(mask):
            aux['gm'] = torch.sqrt(torch.mean(torch.square(cv_forces[mask] - forces[mask])))
        else:
            aux['gm'] = 0.0

        aux['gd'] = torch.mean(torch.norm(cv_forces, dim=-1))
        #mask = torch.sum(torch.isnan(forces), dim=1)
        #if torch.any(mask):
        #    aux['gd'] = torch.mean(torch.abs(cv_forces[mask]))
        #else:
        #    aux['gd'] = 0.0
