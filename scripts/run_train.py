...
from phace.modules import PhACE, extract_kwargs
from phace.modules.loss import CrossEntropyPhaseLoss
import phace.tools
...
from phace.data import AtomicData
...
def main() -> None:
    ...
    if args.model == "AtomicDipolesMACE":
        ...
        logging.info('using the following weights for the phases: {}'.format(phase_weights))
    ...
    train_loader = torch_geometric.dataloader.DataLoader(
        dataset=[
            AtomicData.from_config(config, z_table=z_table, p_table=p_table, cutoff=model_kwargs['r_max'])
    ...
    if args.loss == "weighted":
    ...
    elif args.loss == "weighted_l1":
        from transfermace.modules.loss import WeightedL1EnergyForcesLoss
        loss_fn = WeightedL1EnergyForcesLoss(
        ...
        loss_fn = CrossEntropyPhaseLoss(
                weight=[phase_weights[p] for p in p_table.phases],
                label_smoothing=args.label_smoothing,
                entropy_decay=args.logit_entropy,
                #CV_gradient_decay=args.CV_gradient_decay,
                #CV_forces_matching=args.CV_forces_matching,
                #CV_stress_matching=args.CV_stress_matching,
                )
        loss_fn.to(device)
    else:
    ...
    feature_layers = ast.literal_eval(args.feature_layers)
    classifier_layers = ast.literal_eval(args.classifier_layers)
    model = PhACE.from_model(
            base_model,
    ...
    param_options = dict(
        params=[
            ...
            #    "params": model.selections.parameters(),
            #    "weight_decay": args.weight_decay,
            #},
            {
                "name": "classifier",
